...
'use strict';
const CLINIC = 362;
const PRACTICE_ID = 1311;
const DOCTOR_IDS = [337977, 491046, 691298, 1033012];
...
async function _mrxRequestNotifications() {
...
function _mrxNotify(patient) {
  if (Notification.permission !== 'granted') return;
  if (_notifiedToday.has(patient.patient_id)) return;
  _notifiedToday.add(patient.patient_id);
  const n = new Notification('📅 Book before they leave — Mobility Rx', {
    body: `${patient.name} · last appointment today · book next visits now`,
    tag: `mrx-sched-${patient.patient_id}`,
    requireInteraction: true,
  });
  n.onclick = () => {
    window.focus();
    document.getElementById('_mrx_overlay')?.classList.add('open');
    _mrxDet(patient.patient_id);
    n.close();
  };
}
function _mrxCheckAlerts() {
  (window._mrxData || []).filter(p => p.today_sched_alert).forEach(_mrxNotify);
}

// ── AUTO-REFRESH ──────────────────────────────────────────────────────────────
let _mrxRefreshTimer = null;
function _mrxStartAutoRefresh() {
  if (_mrxRefreshTimer) clearInterval(_mrxRefreshTimer);
  _mrxRefreshTimer = setInterval(() => {
    _mrxLoad(true); // silent refresh
  }, 5 * 60 * 1000); // every 5 min
}

// ── SHEET INTEGRATION ─────────────────────────────────────────────────────────
async function sheetSaveLog(entry) {
  try { await fetch(SHEET_URL, { method: 'POST', body: JSON.stringify({ action: 'log', ...entry }) }); } catch(e) {}
}
async function sheetGetLogs(patient_id) {
  try { const r = await fetch(SHEET_URL + '?patient_id=' + patient_id); return await r.json(); } catch(e) { return []; }
}
async function sheetSaveFlag(entry) {
  try { await fetch(SHEET_URL, { method: 'POST', body: JSON.stringify({ action: 'flag', ...entry }) }); } catch(e) {}
}
...
if (window._mrxRefreshTimer) { clearInterval(window._mrxRefreshTimer); window._mrxRefreshTimer = null; }
...
@keyframes _mrx_p{0%,100%{opacity:1}50%{opacity:.3}}
@keyframes _mrx_pulse{0%,100%{box-shadow:0 0 0 2px #F97316}50%{box-shadow:0 0 0 4px #F97316,0 0 12px rgba(249,115,22,.5)}}
@keyframes _mrx_pulse_red{0%,100%{box-shadow:0 0 0 2px #C13535}50%{box-shadow:0 0 0 5px #C13535,0 0 16px rgba(193,53,53,.6)}}
...
._mnotif{font-family:'DM Sans',system-ui,sans-serif;font-size:11px;font-weight:500;padding:4px 10px;border-radius:8px;border:1px solid #F97316;background:#FFF7ED;color:#C2410C;cursor:pointer}
._mnotif:hover{background:#FED7AA}
._mnotif.on{border-color:#B0D9C0;background:#EDF6F1;color:#1A6B3A}
._mmet{display:grid;grid-template-columns:repeat(4,1fr);gap:5px;padding:9px 12px;flex-shrink:0;background:#F4F3EF;border-bottom:1px solid #E0DDD6}
...
._mrow.today-alert{background:#FFF7ED;border-left:3px solid #F97316}
._mrow.today-alert:hover{background:#FED7AA}
...
._mrefbar{background:#FFF7ED;border-bottom:1px solid #F97316;padding:5px 12px;font-size:11px;color:#C2410C;font-weight:600;text-align:center;display:none}
._mrefbar.show{display:block}
...
overlay.innerHTML = `
  <div class="_mh">
    ...
    <div style="display:flex;align-items:center;gap:5px">
      ...
      <button class="_mnotif" id="_mrxnotifbtn" onclick="_mrxToggleNotif()" title="Enable desktop alerts">🔔</button>
  ...
  <div class="_mrefbar" id="_mrxrefbar"></div>
  <div class="_mmet">
...
// ── NOTIFICATION TOGGLE ───────────────────────────────────────────────────────
window._mrxToggleNotif = async function() {
  const granted = await _mrxRequestNotifications();
  const btn2 = document.getElementById('_mrxnotifbtn');
  if (btn2) {
    btn2.classList.toggle('on', granted);
    btn2.title = granted ? 'Desktop alerts ON' : 'Enable desktop alerts';
  }
  if (granted) {
    new Notification('Mobility Rx POC Tracker', {
      body: 'Booking alerts are enabled. You will be notified when a patient needs to be booked.',
      tag: 'mrx-notif-test',
    });
  }
};
...
if (Notification.permission === 'granted') {
  setTimeout(() => {
    const nb = document.getElementById('_mrxnotifbtn');
    if (nb) { nb.classList.add('on'); nb.title = 'Desktop alerts ON'; }
  }, 100);
}
...
function _mrxSyncTabs() {
  ...
  const tabs = ['all','non_compliant','expired','today_alert','one_appt_left','no_future_appt','at_risk'];
  const idx = tabs.indexOf(window._mrxFlt);
  if (idx >= 0) document.querySelectorAll('._mft')[idx]?.classList.add('on');
  ['_mrxme','_mrxmexp','_mrxmc','_mrxmk','_mrxmns','_mrxmna','_mrxmar'].forEach(id => document.getElementById(id)?.classList.remove('active'));
  const mm = { non_compliant:'_mrxme', expired:'_mrxmexp', recert_due:'_mrxmc', certified:'_mrxmk', today_alert:'_mrxmns', no_future_appt:'_mrxmna', at_risk:'_mrxmar' };
...
window._mrxRender = function() {
  ...
  if (nsTile) nsTile.style.animation = todayCount > 0 ? '_mrx_pulse_red 1.5s infinite' : '';

  // Refresh bar for silent refreshes
  const refBar = document.getElementById('_mrxrefbar');
  if (refBar && todayCount > 0 && !overlay.classList.contains('open')) {
    refBar.textContent = `📅 ${todayCount} patient${todayCount>1?'s':''} need booking today`;
    refBar.classList.add('show');
    btn.style.animation = '_mrx_pulse_red 1.5s infinite';
  } else if (refBar) {
    refBar.classList.remove('show');
    if (todayCount === 0) btn.style.animation = '';
  }
  ...
  rows.sort((a, b) => {
    if (a.today_sched_alert && !b.today_sched_alert) return -1;
    if (!a.today_sched_alert && b.today_sched_alert) return 1;
    const sp = (a.sched_priority||4) - (b.sched_priority||4);
    if (sp !== 0) return sp;
    const ra = RK[a.risk] ?? 3, rb2 = RK[b.risk] ?? 3;
  ...
  list.innerHTML = rows.map(p => {
    ...
    const rowCls = p.today_sched_alert ? 'today-alert' : '';
    return `<div class="_mrow ${rowCls}${p.patient_id === window._mrxDid ? ' sel' : ''}" onclick="_mrxDet(${p.patient_id})">
      ...
      <div class="_mrb">
        ...
        <div style="display:flex;gap:3px;margin-top:3px;flex-wrap:wrap;align-items:center">
          ...
          ${p.today_sched_alert ? '<span class="_mbg" style="background:#FEF2F2;color:#C13535;border:2px solid #C13535;font-weight:700;animation:_mrx_pulse_red 1.5s infinite">📅 BOOK TODAY</span>' : ''}
          ${p.one_appt_left && !p.today_sched_alert ? '<span class="_mbg b_orange">1 appt left</span>' : ''}
          ${p.at_risk ? '<span class="_mbg" style="background:#FEF2F2;color:#7F1D1D;border:1px solid #FECACA;font-weight:700">⚠ At Risk</span>' : ''}
          ${p.no_future_appt ? '<span class="_mbg" style="background:#F3E8FF;color:#6B21A8;border:1px solid #D8B4FE">No future appt</span>' : ''}
  ...
  det.innerHTML = `
    ...
    <div class="_mdc2">
      ...
      <div class="_mdcol">
        ...
        <div class="_mdr"><span class="_mdk">Future appts</span><span class="_mdv" style="color:${p.sched_priority===1?'#C13535':p.sched_priority===2?'#C2410C':'#1A6B3A'}">${p.sched_priority===1?'None scheduled':p.sched_priority===2?'Last appt left':'Scheduled'}</span></div>
    ...
    <div class="_mcl">
      ...
      <div class="_mform" style="margin-top:8px">
        <div class="_mfrow">
          ...
          <input type="text" id="_mrxuser" placeholder="Initials (e.g. PL)">
        </div>
      ...
      <div id="_mrxflagform" style="display:none;margin-top:10px;padding-top:10px;border-top:1px solid #EFEDE8">
        ...
        <div class="_mform">
          <input type="text" id="_mrxflagwhat" placeholder="What does tracker show?">
          <input type="text" id="_mrxflagreal" placeholder="What is it actually?">
...
window._mrxOpenFlag = (pid) => { const el = document.getElementById('_mrxflagform'); if (el) el.style.display = el.style.display === 'none' ? 'block' : 'none'; };
window._mrxSubmitFlag = async (pid, name) => {
  ...
  await sheetSaveFlag({ patient_id: pid, patient_name: name, date: new Date().toLocaleString('en-US', { month: 'short', day: 'numeric', year: 'numeric', hour: 'numeric', minute: '2-digit' }), tracker_shows: what, actual_value: real, flagged_by: by });
...
window._mrxAddLog = async function(pid) {
  ...
  const entry = { patient_id: pid, patient_name: p?.name || '', date: new Date().toLocaleString('en-US', { month: 'short', day: 'numeric', year: 'numeric', hour: 'numeric', minute: '2-digit' }), outcome, note: note || '', initials: user || '' };
...
// ── DATA LOADING ──────────────────────────────────────────────────────────────
window._mrxLoad = async function(silent) {
  const refBtn = document.getElementById('_mrxref');
  if (refBtn) refBtn.disabled = true;
  const setStatus = (msg) => { if (!silent) { const el = document.getElementById('_mrxlist'); if (el) el.innerHTML = '<div class="_mload">'+msg+'</div>'; } };
  if (!silent) setStatus('Pulling patients from Spry…');
  ...
  try {
    ...
    for (let pg = 1; pg <= 200; pg++) {
      ...
      if (!silent) setStatus(`Loading patients: ${patients.length}${totalPages ? ' of ' + (totalPages * 10) : ''}…`);
      if (totalPages !== null && pg >= totalPages) break;
      if (batch.length === 0) break;
    }

    // ── STEP 2: POC data ──────────────────────────────────────────────────────
    if (!silent) setStatus(`Loading POC data 0/${patients.length}…`);
    const pocMap = {};
    for (let i = 0; i < patients.length; i += 10) {
      ...
      const res = await Promise.all(batch.map(async p => {
        try {
          ...
          return { patient_id: p.patient_id, name: p.name, first_name: p.first_name, last_name: p.last_name, mobile: p.mobile, referring: p.additional_details?.referring_physician_name || '', insurance, ins_type: insType, last_appt: p.additional_details?.last_appointment_date, poc_status: lat?.poc_status || null, poc_created_at: lat?.poc_created_at || null, poc_expiration_date: lat?.poc_expiration_date || null, treatment_weeks: lat?.treatment_duration_in_weeks || null, visit_frequency: lat?.visit_frequency || null, case_title: lat?.case_title || null, signed_doc: !!lat?.signed_poc_document_url, poc_count: list2.length, log_count: 0 };
      ...
      if (!silent) setStatus(`POC data: ${Object.keys(pocMap).length}/${patients.length}`);
    }
    ...
    // ── STEP 5.6: Enhancement reports ────────────────────────────────────────
    if (!silent) setStatus('Loading compliance reports…');
    const fotPids = new Set(), fotDetails = {}, scriptPids = new Set(), oneApptPids = new Set();
    try {
      const now = new Date();
      const todayStr = now.toISOString().split('T')[0];
      const tomorrowStr = new Date(now.getTime() + 864e5).toISOString().split('T')[0];
      const baseParams = { period: '0_days_day', from: todayStr + 'T07:00:00', to: tomorrowStr + 'T06:59:59', clinic_ids: [CLINIC], organisation_id: 229, size: 1000000 };
      const callReport = async (kpi, extraParams) => {
        const body = JSON.stringify({ kpi, params: { ...baseParams, ...extraParams, kpi } });
        const r = await fetch('/apis/analytics/emr/report', { method: 'POST', headers: { ...h, 'Content-Type': 'application/json' }, body });
        if (!r.ok) return [];
        const j = await r.json();
      ...
      fotSeries.forEach(v => { if (v[0]) { fotPids.add(String(v[0])); fotDetails[String(v[0])] = v[4] || 'Outcome measure'; } });
    ...
    window._mrxData = pocData.map(p => {
      ...
      const fot_pending = fotPids.has(pid), fot_name = fotDetails[pid] || null;
      const rx_pending = scriptPids.has(pid);
    ...
    _mrxRender();
    _mrxCheckAlerts();
    _mrxStartAutoRefresh();
  } catch(e) {
    if (!silent) { const el = document.getElementById('_mrxlist'); if (el) el.innerHTML = `<div class="_mload" style="color:#C13535">Error: ${e.message}</div>`; }
  }
